<?
    ...
    $_SESSION['ACESSO'] = 'Balanço Natureza de Operação';
    include __DIR__."/../../sgbd/acesso.php";
?>
<div id="cpc-topo" class="card my-2">
    <div class="row card-head h5 text-center mt-1">
        <div class="col-12 d-flex justify-content-evenly">
            <button type="button" class="btn btn-outline-primary btn-sm cpcnoprint" data-bs-toggle="offcanvas" data-bs-target="#formbalanco_natop" aria-controls="formbalanco_natop">
...
<form action="." method="post" class="btn-group d-flex justify-content-between mb-3 offcanvas offcanvas-start" role="group" aria-label="Button group with nested dropdown" tabindex="-1" id="formbalanco_natop" name="formbalanco_natop" aria-labelledby="offcanvasLabel">
    ...
    <div class="offcanvas-body">
        ...
        <div class="mt-4 d-flex justify-content-center">
            <button class="btn btn-outline-success w-100" id="balanco_depto" onclick="sqlitem($('#formbalanco_natop'),'financeiro/relatorios/balanco_custos','cpcMain');">Atualizar</button>
        </div>
    </div>
</form>
    ...
    <table id="balanco_depto" class="container table table-hover table-sm tabelareport" style="min-width:1440px">
        ...
        <tbody id="cpc-autoheightgrid" class="text-gray-600 tabelabody">
            <?
                ...
                $sql = "select movcustos.custos, custos.a_nome departamento, sum(iif(movimen.tipo = 'E',movcustos.valor,0)) recebimento, sum(iif(movimen.tipo = 'S',movcustos.valor,0)) pagamento from MOVIMEN inner join movcustos on (movimen.NUMERO = movcustos.numero) inner join CUSTOS on (custos.custos = movcustos.custos) where movimen.data1 between  '".str_replace('/','.',$_POST['data-ini'])."' and '".str_replace('/','.',$_POST['data-fim'])."' group by 1,2";
                $sql .= "order by 2";
                $pagamento = 0;
                $recebimento = 0;
                $cont = 0;
...
<div class="loadcpc" onclick="
    ...
    $('#formbalanco_natop').offcanvas('show');
"></div>
<? } ?>