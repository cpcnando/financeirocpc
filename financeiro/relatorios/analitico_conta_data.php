<?
    ...
    $_SESSION['ACESSO'] = 'Conta Corrente por Data';
...
<form action="." method="post" class="btn-group d-flex justify-content-between mb-3 offcanvas offcanvas-start" role="group" aria-label="Button group with nested dropdown" tabindex="-1" id="formfiltro" name="formfiltro" aria-labelledby="offcanvasLabel">
    ...
    <div class="offcanvas-body">    
        ...
        <button class="btn btn-success m-1 w-100" for="Atualizar" id="atualizar_filtro" onclick="sqlitem($('#formfiltro'),'financeiro/relatorios/analitico_conta_data','cpcMain','#listabaixa');">Atualizar</button>
    </div>
</form>
...
<div id="cpc-autoheight" style="overflow: auto;">
    <table id="listaanaliticoconta" class="table table-hover table-sm tabelareport" style="min-width:1024px">
        ...
        <?
                ...
                $sql = "select emissao, conta, custos, indice, max(baixa) baixa, max(historico) historico, sum(receita) receita, sum(despesa) despesa from financeirogeral('where emissao between ''".str_replace('/','.',$_POST['data1'])."'' and ''".str_replace('/','.',$_POST['data2'])." 23:59:59'' and tipo in (''".$tipo1."'',''".$tipo2."'')') where 1 = 1";
                ...
                $sql = $sql.' group by 1,2,3,4 order by emissao, conta, indice';
            ...
            foreach ($financeiro->query($sql) as $rowanalitico) 
            {
                if ($data != $rowanalitico['EMISSAO']) {
                    $data = $rowanalitico['EMISSAO'];
                    $sql = "select * from saldotipogeral(".$rowanalitico['CONTA'].",'".date("d.m.Y", strtotime(date("d.m.Y",strtotime($rowanalitico['EMISSAO'])). ' - 1  days'))." 23:59:59','0','CONTA')";
                    
                foreach ($financeiro->query($sql) as $rowconta) 
                {if (($cont === 1) && ($rowconta['VALOR'])) $total = $rowconta['VALOR'];}
        ...
        <tr class="table-active">
            <td colspan=6 class="h3"><? echo date('d/m/Y', strtotime($rowanalitico['EMISSAO'])) ?></td>
            <td colspan=2 class="text-end align-middle"><? echo number_format($rowconta['VALOR'], 2,',','.'); ?></td>
        </tr>

        <? } ?>
        <tr>
            <td class="text-end"><? echo $cont++; ?></td>
            <td class="text-end"><div style="cursor:pointer" onclick="$('#abaitem').val('aba-dados'); showMain(<? echo $rowanalitico['INDICE'] ?>,'telapopup','financeiro/movimento.php?indice=');" data-bs-toggle="tooltip" data-bs-placement="top" title="ir para o Movimento"><?php echo $rowanalitico['INDICE'] ?></div></td>
            <td><? echo executasqlcampo("select a_nome from contas where conta =0".$rowanalitico['CONTA'],$financeiro); ?></td>
            <td><? echo executasqlcampo("select a_nome from custos where custos =0".$rowanalitico['CUSTOS'],$financeiro); ?></td>
            <td class="lh-1"><? echo $rowanalitico['HISTORICO'] ?></td>
            <td class="text-end"><? if ($rowanalitico['RECEITA']) { $entrada += $rowanalitico['RECEITA']; echo number_format($rowanalitico['RECEITA'], 2,',','.'); if ($rowanalitico['RECEITA']) $total += $rowanalitico['RECEITA']; } ?></td>
            <td class="text-end"><? if ($rowanalitico['DESPESA']) { $saida += $rowanalitico['DESPESA']; echo number_format($rowanalitico['DESPESA'], 2,',','.'); if ($rowanalitico['DESPESA']) $total -= $rowanalitico['DESPESA']; } ?></td>
