<?
    ...
    $_SESSION['ACESSO'] = 'Analítico por Conta Corrente';
...
<form action="." method="post" class="btn-group d-flex justify-content-between mb-3 offcanvas offcanvas-start" role="group" aria-label="Button group with nested dropdown" tabindex="-1" id="formfiltro" name="formfiltro" aria-labelledby="offcanvasLabel">
    ...
    <div class="offcanvas-body">    
        ...
        <button class="btn btn-success m-1 w-100" for="Atualizar" id="atualizar_filtro" onclick="sqlitem($('#formfiltro'),'financeiro/relatorios/analitico_conta','cpcMain','#listabaixa');">Atualizar</button>
    </div>
</form>
...
<div id="cpc-autoheight" style="overflow: auto;">
    <table id="listaanaliticoconta" class="table table-hover table-sm tabelareport" style="min-width:1024px">
        ...
        <?
                ...
                $sql = "select conta, indice, max(emissao) emissao, max(baixa) baixa, max(historico) historico, sum(receita) receita, sum(despesa) despesa from financeirogeral('where emissao between ''".str_replace('/','.',$_POST['data1'])."'' and ''".str_replace('/','.',$_POST['data2'])." 23:59:59'' and tipo in (''".$tipo1."'',''".$tipo2."'')') where 1 = 1";
                ...
                // }
                $sql = $sql.' group by 1,2 order by conta, emissao ,indice';
            ...
            {
                if ($conta != $rowanalitico['CONTA']) {
                    $conta = $rowanalitico['CONTA'];
                    $sql = "select * from saldotipo(".$rowanalitico['CONTA'].",'".date("d.m.Y", strtotime(date("d.m.Y",strtotime($rowanalitico['EMISSAO'])). ' - 1  days'))." 23:59:59','".($_POST['tipo_data'] ?? '1')."')";
        ...
        <tr class="table-active">
            <td colspan=6 class="text-start text-muted fw-bold fs-7 gs-0"><? echo $rowconta['DESCRICAO']; ?></td>
            <td colspan=2 class="fw-bold fs-7 gs-0 text-end align-middle"><? echo number_format($rowconta['VALOR'], 2,',','.'); ?></td>
        </tr>
        ...
        <tr>
            ...
            <td><? if ($rowanalitico['EMISSAO'] != '') echo  date('d/m/Y', strtotime($rowanalitico['EMISSAO'])) ?></td>
            <td><? if ($rowanalitico['BAIXA'] != '') echo  date('d/m/Y', strtotime($rowanalitico['BAIXA'])) ?></td>
