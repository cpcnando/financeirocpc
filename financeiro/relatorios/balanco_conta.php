<?
    ...
    $_SESSION['ACESSO'] = 'Balanço Contas Corrente';
    include __DIR__."/../../sgbd/acesso.php";
?>
<div id="cpc-topo" class="card my-2">
    <div class="row card-head h5 text-center mt-1">
        <div class="col-12 d-flex justify-content-evenly">
            <button type="button" class="btn btn-outline-primary btn-sm cpcnoprint" data-bs-toggle="offcanvas" data-bs-target="#formbalanco_conta" aria-controls="formbalanco_conta">
            ...
            <div class="input-group justify-content-center">
                    ...
                    <input type="text" name="buscar" id="buscar" class="form-control form-control-sm cpcnoprint" onkeyup="filtraTabela($(this).val(),'balanco_conta')" placeholder="Pesquisar..." style="min-width:200px; max-width:800px">
            </div>
...
<form id="formbalanco_conta" name="formbalanco_conta" action="." method="post" class="btn-group d-flex justify-content-between mb-3 offcanvas offcanvas-start" role="group" aria-label="Button group with nested dropdown" tabindex="-1" aria-labelledby="offcanvasLabel">
    <input type="hidden" name="tipodml" value="balanco_conta">
    ...
    <div class="offcanvas-body">
        ...
        <div class="col-12" role="group">
            ...
            <select class="form-select cpcsave" name="status" id="status">
                <option>Todos</option>
        ...
        <div class="mt-4 d-flex justify-content-center">
            <button class="btn btn-outline-success w-100" id="balanco_conta" onclick="sqlitem($('#formbalanco_conta'),'financeiro/relatorios/balanco_conta','cpcMain');">Atualizar</button>
        </div>
    </div>
</form>

<div id="cpc-autoheight" style="overflow: auto;">
    <table id="balanco_conta" class="container table table-hover table-sm tabelareport" style="min-width:1440px">
        <thead class="mb-3 tabelahead">
            <tr class="text-start text-muted fw-bold fs-7 gs-0">
                <th class="text-end">Cód</th>
                <th>Descrição</th>
                <th class="text-end">Recebimentos</th>
        ...
        <tbody id="cpc-autoheightgrid" class="text-gray-600 tabelabody">
            <?
                ...
                $sql = "select movimen.conta, contas.a_nome nomeconta, sum(iif(movimen.tipo = 'E',movimen.valor,0)) recebimento, sum(iif(movimen.tipo = 'S',movimen.valor,0)) pagamento from MOVIMEN inner join contas on (contas.CONTA = movimen.conta) where movimen.data1 between  '".str_replace('/','.',$_POST['data-ini'])."' and '".str_replace('/','.',$_POST['data-fim'])."' group by 1,2";
                $sql .= "order by 2";
                $recebimento = 0;
                $pagamento = 0;
                $cont = 0;
...
<div class="loadcpc" onclick="
    ...
    $('#formbalanco_conta').offcanvas('show');
"></div>
<? } ?>