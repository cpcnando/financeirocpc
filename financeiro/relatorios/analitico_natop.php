<?
    ...
    $_SESSION['ACESSO'] = 'Analítico por Natureza de Operação';
...
<form action="." method="post" class="btn-group d-flex justify-content-between mb-3 offcanvas offcanvas-start" role="group" aria-label="Button group with nested dropdown" tabindex="-1" id="formfiltro" name="formfiltro" aria-labelledby="offcanvasLabel">
    ...
    <div class="offcanvas-body">    
        <div class="col-12">
            ...
            <input type="date" class="form-control cpcsave" placeholder="" name="data1" value="<? echo date("Y-m-d", strtotime(date("m.d.y"). ' - 7  days')) ?>" required>
        </div>
        <div class="col-12">
            <label class="form-label">Até</label>
            <input type="date" class="form-control cpcsave" placeholder="" name="data2" value="<? echo date('Y-m-d') ?>">
        </div>
        <div class="col-12">
        ...
        <div class="col-12">
            <label class="form-label">Conciliado</label>
        ...
        <button class="btn btn-success m-1 w-100" for="Atualizar" id="atualizar_filtro" onclick="sqlitem($('#formfiltro'),'financeiro/relatorios/analitico_natop','cpcMain','#listabaixa');">Atualizar</button>
    </div>
</form>
...
<div id="cpc-autoheight" style="overflow: auto;">
    <table id="listaanaliticoconta" class="table table-hover table-sm tabelareport" style="min-width:1024px">
        ...
        <?
                ...
                $sql = "select custos, indice, max(emissao) emissao, max(baixa) baixa, max(historico) historico, sum(receita) receita, sum(despesa) despesa from financeirogeral('where emissao between ''".str_replace('/','.',$_POST['data1'])."'' and ''".str_replace('/','.',$_POST['data2'])." 23:59:59'' and tipo in (''".$tipo1."'',''".$tipo2."'')') where 1 = 1";
                ...
                // }
                $sql = $sql.' group by 1,2 order by custos, emissao ,indice';
            ...
            foreach ($financeiro->query($sql) as $rowanaliticocustos) 
            {
                if ($custos != $rowanaliticocustos['CUSTOS']) {
                    $custos = $rowanaliticocustos['CUSTOS'];
                    $sql = "select * from saldotipogeral(".$rowanaliticocustos['CUSTOS'].",'".date("d.m.Y", strtotime(date("d.m.Y",strtotime($rowanaliticocustos['EMISSAO'])). ' - 1  days'))." 23:59:59','".($_POST['tipo_data'] ?? '1')."','CUSTOS')";
                    
                foreach ($financeiro->query($sql) as $rownatop) 
                {if ($rownatop['VALOR']) $total = $rownatop['VALOR'];}
        ...
        <tr class="table-active">
            <td colspan=6 class="h3"><? echo $rowanaliticocustos['CUSTOS']. ' '.$rownatop['DESCRICAO']; ?></td>
            <td colspan=2 class="text-end align-middle"><? echo number_format($rownatop['VALOR'], 2,',','.'); ?></td>
        </tr>

        <? } ?>
        <tr>
            <td class="text-end"><? echo $cont++; ?></td>
            <td class="text-end"><div style="cursor:pointer" onclick="$('#abaitem').val('aba-dados'); showMain(<? echo $rowanaliticocustos['INDICE'] ?>,'telapopup','financeiro/movimento.php?indice=');" data-bs-toggle="tooltip" data-bs-placement="top" title="ir para o Movimento"><?php echo $rowanaliticocustos['INDICE'] ?></div></td>
            <td><? if ($rowanaliticocustos['EMISSAO'] != '') echo  date('d/m/Y', strtotime($rowanaliticocustos['EMISSAO'])) ?></td>
            <td><? if ($rowanaliticocustos['BAIXA'] != '') echo  date('d/m/Y', strtotime($rowanaliticocustos['BAIXA'])) ?></td>
            <td class="lh-1"><? echo $rowanaliticocustos['HISTORICO'] ?></td>
            <td class="text-end"><? if ($rowanaliticocustos['RECEITA']) { $entrada += $rowanaliticocustos['RECEITA']; echo number_format($rowanaliticocustos['RECEITA'], 2,',','.'); if ($rowanaliticocustos['RECEITA']) $total += $rowanaliticocustos['RECEITA']; } ?></td>
            <td class="text-end"><? if ($rowanaliticocustos['DESPESA']) { $saida += $rowanaliticocustos['DESPESA']; echo number_format($rowanaliticocustos['DESPESA'], 2,',','.'); if ($rowanaliticocustos['DESPESA']) $total -= $rowanaliticocustos['DESPESA']; } ?></td>
